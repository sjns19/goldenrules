...
$(document).ready(function () {
  ...
  if (elementExists("#login-form")) {
      ...
      $.post({
        ...
        .done(function (response) {
          ...
          if (res["status"] === "success") {
            ...
            w.location =
              refSRC !== null && refFor == "paid_membership"
                ? decodeURIComponent(refSRC)
                : w.location.origin;
          }
        })
        .fail(function () {
          $form.setFormDisability(false, "login", $loginAlert);
          $loginAlert.showError(
    ...
    $doc.on("input", loginInputs, function () {
      ...
      $(loginInputs).each(function () {
        if ($(this).val() == "") allEmpty = true;
      });

      if ($loginAlert.html() !== "") $loginAlert.showError(false);
  ...
  if (elementExists("#reg-form")) {
    ...
    $doc.on("input", "#reg-password-retype", function () {
      ...
      if (len >= 1 && value !== $("#reg-password").val()) status = true;
    ...
    $doc.on("input", regInputs, function () {
      ...
      if ($regAlert.html() !== "") $regAlert.showError(false);
    });
  }
});
