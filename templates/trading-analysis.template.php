<?php if (!defined(basename(__FILE__) . '-included')) return http_response_code(404); ?>
		...
		<?php for ($p = 1, $total = $pages; $p <= $total; $p++): ?>
			<?php if ($p == $active_page): $dots = true; ?>
							<li class="pagination-item"><a href="" class="pagination-link active"><?= $p ?></a></li>
			<?php else: ?>
		...
		<?php if ($active_page < $pages || -1 == $pages): ?>
							<li class="pagination-item"><a href="/news/?page=<?= ($active_page + 1) ?>" class="pagination-link" title="Next">»</a></li>
		<?php endif; ?>
						</ul>
		<?php endif; ?>
					</div>
